...
def SCMDecoratedReductionMethod(ReductionMethod_DerivedClass):
    class SCMDecoratedReductionMethod_Class(ReductionMethod_DerivedClass):
        def __init__(self, truth_problem):
            ...
            def error_analysis(self, N=None):
                ...
                self.SCM_reduction_method[i].error_analysis(N)
                # ..., and then call the parent method.
    ...
    class _SCMReductionMethod(ParametrizedProblem):
        ...
        def set_xi_train(self, ntrain, enable_import=True, sampling="random"):
            ...
            if import_successful == False:
                if os.path.exists(self.reduced_operators_folder):
                    shutil.rmtree(self.reduced_operators_folder)
                os.makedirs(self.reduced_operators_folder)
        ...
        def offline(self):
            ...
            folders = (self.snap_folder, self.basis_folder, self.reduced_operators_folder, self.post_processing_folder)
            for f in folders:
                if not os.path.exists(f):
                    os.makedirs(f)
            
            # Save M_e and M_p
            np.save(self.reduced_operators_folder + "M_e", self.M_e)
            np.save(self.reduced_operators_folder + "M_p", self.M_p)
        ...
        def compute_bounding_box(self):
            ...
            # Save to file
            np.save(self.reduced_operators_folder + "B_min", self.B_min)
            np.save(self.reduced_operators_folder + "B_max", self.B_max)
        ...
        def update_C_J(self):
            ...
            # Save to file
            np.save(self.reduced_operators_folder + "C_J", self.C_J)
            np.save(self.reduced_operators_folder + "complement_C_J", self.complement_C_J)
        ...
        def greedy(self):
            ...
            np.save(self.reduced_operators_folder + "alpha_LB_on_xi_train", self.alpha_LB_on_xi_train)
        ...
        def load_reduced_data_structures(self):
            if len(np.asarray(self.B_min)) == 0: # avoid loading multiple times
                self.B_min = np.load(self.reduced_operators_folder + "B_min.npy")
            if len(np.asarray(self.B_max)) == 0: # avoid loading multiple times
                self.B_max = np.load(self.reduced_operators_folder + "B_max.npy")
            if len(np.asarray(self.C_J)) == 0: # avoid loading multiple times
                self.C_J = np.load(self.reduced_operators_folder + "C_J.npy")
                self.N = len(self.C_J)
            if len(np.asarray(self.complement_C_J)) == 0: # avoid loading multiple times
                self.complement_C_J = np.load(self.reduced_operators_folder + "complement_C_J.npy")
            if len(np.asarray(self.alpha_J)) == 0: # avoid loading multiple times
                self.alpha_J = np.load(self.reduced_operators_folder + "alpha_J.npy")
            if len(np.asarray(self.alpha_LB_on_xi_train)) == 0: # avoid loading multiple times
                self.alpha_LB_on_xi_train = np.load(self.reduced_operators_folder + "alpha_LB_on_xi_train.npy")
            if len(np.asarray(self.UB_vectors_J)) == 0: # avoid loading multiple times
                self.UB_vectors_J = np.load(self.reduced_operators_folder + "UB_vectors_J.npy")
            if not self.M_e: # avoid loading multiple times
                self.M_e = np.load(self.reduced_operators_folder + "M_e.npy")
                self.M_e = int(self.M_e)
            if not self.M_p: # avoid loading multiple times
                self.M_p = np.load(self.reduced_operators_folder + "M_p.npy")
