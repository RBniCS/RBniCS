...
def matrix_mul_vector(matrix, vector):
    if isinstance(vector, (rbnics.backends.dolfin.Function.Type(), Operator)):
...
def vectorized_matrix_inner_vectorized_matrix(matrix, other_matrix):
    assert isinstance(matrix, rbnics.backends.dolfin.Matrix.Type()) or (isinstance(matrix, Form) and len(matrix.arguments()) is 2)
    assert isinstance(other_matrix, rbnics.backends.dolfin.Matrix.Type()) or (isinstance(other_matrix, Form) and len(other_matrix.arguments()) is 2)
