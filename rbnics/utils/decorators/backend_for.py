...
def BackendFor(library, inputs=None, replaces=None, replaces_if=None):
    def BackendFor_Decorator(Class):
        ...
        if inspect.isclass(getattr(_cache, Class.__name__)): # added by @AbstractBackend, first time @BackendFor is called for this class name
...
def backend_for(library, inputs=None, replaces=None, replaces_if=None):
    def backend_for_decorator(function):
        ...
        if inspect.isfunction(getattr(_cache, function.__name__)): # added by @abstract_backend, first time @backend_for is called for this function name
