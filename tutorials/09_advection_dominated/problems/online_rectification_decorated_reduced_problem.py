...
def OnlineRectificationDecoratedReducedProblem(EllipticCoerciveReducedProblem_DerivedClass):
    ...
    class OnlineRectificationDecoratedReducedProblem_Class(EllipticCoerciveReducedProblem_DerivedClass):
        def __init__(self, truth_problem, **kwargs):
            ...
            # The difference between this list and greedy_selected_parameters in the reduction method is that
            # this one also stores the initial parameter
        ...
        def _init_operators(self, current_stage="online"):
            ...
            if current_stage == "online":
                ...
                assert len(self.online_solve_kwargs_with_rectification) == len(
                    self.online_solve_kwargs_without_rectification)
                self.operator["projection_reduced_snapshots"] = OnlineNonHierarchicalAffineExpansionStorage(len(
                    self.online_solve_kwargs_with_rectification))
        ...
        def build_reduced_operators(self, current_stage="offline"):
            if current_stage == "offline_rectification_postprocessing":
                ...
                self.operator["projection_truth_snapshots"] = self.assemble_operator(
                    "projection_truth_snapshots", "offline_rectification_postprocessing")
                print("build projection reduced snapshots for rectification")
                self.operator["projection_reduced_snapshots"] = self.assemble_operator(
                    "projection_reduced_snapshots", "offline_rectification_postprocessing")
            ...
            if term == "projection_truth_snapshots":
                ...
                if current_stage == "online":  # load from file
                    ...
                    for n in range(1, self.N + 1):
                        ...
                        for (i, snapshot_i) in enumerate(self.snapshots[:n]):
                            ...
                            solver = LinearSolver(inner_product_n, projected_truth_snapshot_i,
                                                  transpose(basis_functions_n) * inner_product * snapshot_i)
                ...
                if current_stage == "online":  # load from file
                    ...
                    for n in range(1, self.N + 1):
                        ...
                        projection_reduced_snapshots_expansion = OnlineAffineExpansionStorage(len(
                            self.online_solve_kwargs_without_rectification))
                        for (q, online_solve_kwargs) in enumerate(self.online_solve_kwargs_without_rectification):
                            ...
                            print("\t\tonline solve options " + str(dict(
                                self.online_solve_kwargs_with_rectification[q])) + ": "
        ...
        def _solve(self, N, **kwargs):
            ...
            if kwargs["online_rectification"]:
                ...
                solver = LinearSolver(
                    self.operator["projection_reduced_snapshots"][:N, :N][q], intermediate_solution,
                    self._solution.vector())
                solver.set_parameters(self._linear_solver_parameters)
                solver.solve()
                self._solution.vector()[:] = (
                    self.operator["projection_truth_snapshots"][:N, :N][0] * intermediate_solution)
