...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def compute_theta(self, term):
        ...
        if term == "a":
            ...
            if self.stabilized:
                delta = self.delta
                theta_a3 = - delta*mu2*cos(mu1)
                theta_a4 = - delta*mu2*sin(mu1)
                theta_a5 = delta*cos(mu1)**2
                theta_a6 = delta*cos(mu1)*sin(mu1)
                theta_a7 = delta*sin(mu1)**2
            else:
                theta_a3 = 0.0
                theta_a4 = 0.0
                theta_a5 = 0.0
            ...
            if self.stabilized:
                delta = self.delta
                theta_f1 = delta*cos(mu1)
                theta_f2 = delta*sin(mu1)
            else:
                theta_f1 = 0.0
                theta_f2 = 0.0
...
pod_galerkin_method.set_tolerance(1e-7)

# 5. Perform the offline phase
first_mu = (1.0, 4.0)
advection_dominated_problem.set_mu(first_mu)
pod_galerkin_method.initialize_training_set(200)
