...
def pytest_configure(config):
    if config.pluginmanager.hasplugin("benchmark"):
        benchmark_plugin = config.pluginmanager.getplugin("benchmark")
        benchmark_plugin.BenchmarkFixture = BenchmarkFixture
        benchmark_plugin.BenchmarkSession = BenchmarkSession
        
def pytest_runtest_setup(item):
    has_compute_overhad = item.get_marker("compute_overhead") is not None
    has_compute_speedup = item.get_marker("compute_speedup") is not None
    assert not has_compute_overhad or not has_compute_speedup
