...
class ParametrizedExpressionFactory(ParametrizedExpressionFactory_Base):
    def __init__(self, expression, name, mesh):
        ParametrizedExpressionFactory_Base.__init__(self, expression, name)
...
class ParametrizedFunctionApproximation(EIMApproximation):
    def __init__(self, V, expression_type, basis_generation):
        ...
        if expression_type == "Function":
            ...
            EIMApproximation.__init__(self, None, ParametrizedExpressionFactory(f, "f", V.mesh()), "test_eim_approximation_9_function.output_dir", basis_generation)
        elif expression_type == "Vector":
            v = TestFunction(V)
            form = f*v*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ParametrizedTensorFactory(form, "form"), "test_eim_approximation_9_vector.output_dir", basis_generation)
        elif expression_type == "Matrix":
            u = TrialFunction(V)
            v = TestFunction(V)
            form = f*u*v*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ParametrizedTensorFactory(form, "form"), "test_eim_approximation_9_matrix.output_dir", basis_generation)
