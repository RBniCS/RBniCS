...
class ParametrizedFunctionApproximation(EIMApproximation):
    def __init__(self, V, expression_type, basis_generation):
        ...
        if expression_type == "Function":
            ...
            EIMApproximation.__init__(self, None, ProjectedParametrizedExpression(f, V), "test_eim_approximation_6_function.output_dir", basis_generation)
        elif expression_type == "Vector":
            v = TestFunction(V)
            form = inner(f, grad(v))*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ProjectedParametrizedTensor(form, V), "test_eim_approximation_6_vector.output_dir", basis_generation)
        elif expression_type == "Matrix":
            u = TrialFunction(V)
            v = TestFunction(V)
            form = inner(f, grad(u))*v*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ProjectedParametrizedTensor(form, V), "test_eim_approximation_6_matrix.output_dir", basis_generation)
...
parametrized_function_reduction_method.set_xi_train(225, sampling=EquispacedDistribution())
...
parametrized_function_reduction_method.set_xi_test(225)
