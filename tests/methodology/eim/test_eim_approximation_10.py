...
class ParametrizedFunctionApproximation(TimeDependentEIMApproximation):
    def __init__(self, V, expression_type, basis_generation):
        ...
        if expression_type == "Function":
            ...
            TimeDependentEIMApproximation.__init__(self, self.mock_time_dependent_problem, ParametrizedExpressionFactory(f), "test_eim_approximation_10_function.output_dir", basis_generation)
        elif expression_type == "Vector":
            v = TestFunction(V)
            form = f*v*dx
            # Call Parent constructor
            TimeDependentEIMApproximation.__init__(self, self.mock_time_dependent_problem, ParametrizedTensorFactory(form), "test_eim_approximation_10_vector.output_dir", basis_generation)
        elif expression_type == "Matrix":
            u = TrialFunction(V)
            v = TestFunction(V)
            form = f*u*v*dx
            # Call Parent constructor
            TimeDependentEIMApproximation.__init__(self, self.mock_time_dependent_problem, ParametrizedTensorFactory(form), "test_eim_approximation_10_matrix.output_dir", basis_generation)
