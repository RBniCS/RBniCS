...
class ParametrizedFunctionApproximation(EIMApproximation):
    def __init__(self, V, expression_type, basis_generation):
        ...
        if expression_type == "Function":
            ...
            EIMApproximation.__init__(self, None, ProjectedParametrizedExpression(f, V), "test_eim_approximation_7_function.output_dir", basis_generation)
        elif expression_type == "Vector":
            v = TestFunction(V)
            form = inner(f, grad(v))*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ProjectedParametrizedTensor(form, V), "test_eim_approximation_7_vector.output_dir", basis_generation)
        elif expression_type == "Matrix":
            u = TrialFunction(V)
            v = TestFunction(V)
            form = inner(grad(u)*f, grad(v))*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ProjectedParametrizedTensor(form, V), "test_eim_approximation_7_matrix.output_dir", basis_generation)
