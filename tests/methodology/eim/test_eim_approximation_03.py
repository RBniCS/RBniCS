...
import os
import pytest
from dolfin import dx, FunctionSpace, Mesh, TestFunction, TrialFunction
    ...
    * DEIM: test interpolation of form with scalar integrand on a scalar space
    ...
    class ParametrizedFunctionApproximation(EIMApproximation):
        def __init__(self, V, expression_type, basis_generation):
            ...
            f = ParametrizedExpression(mock_problem, "exp( - 2*pow(x[0]-mu[0], 2) - 2*pow(x[1]-mu[1], 2) )", mu=(0., 0.), element=V.ufl_element())
            #
            folder_prefix = os.path.join("test_eim_approximation_03_tempdir", expression_type, basis_generation)
