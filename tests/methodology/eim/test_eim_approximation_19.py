...
import os
import pytest
from dolfin import (assemble, assign, dx, grad, inner, FiniteElement, Function, FunctionSpace, MixedElement, Point,
...
def test_eim_approximation_19(expression_type, basis_generation):
    """
    This test is similar to test 15. The difference with test 15 is that the (solution dependent) function
    to be interpolated is now the gradient of a component of the solution, resulting in a (solution dependent)
    coefficent which is an UFL Grad operator.
    """
    ...
    class MockProblem(ParametrizedProblem):
        def __init__(self, V, **kwargs):
            ...
            ParametrizedProblem.__init__(self, os.path.join(
                "test_eim_approximation_19_tempdir", expression_type, basis_generation, "mock_problem"))
