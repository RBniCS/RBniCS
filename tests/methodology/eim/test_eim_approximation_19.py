...
import os
import pytest
from dolfin import assemble, assign, dx, grad, inner, FiniteElement, Function, FunctionSpace, MixedElement, Point, project, RectangleMesh, SpatialCoordinate, split, sqrt, TestFunction, TrialFunction, VectorElement
...
from rbnics.eim.utils.decorators import StoreMapFromProblemNameToProblem, StoreMapFromProblemToReducedProblem, StoreMapFromProblemToTrainingStatus, StoreMapFromSolutionToProblem, UpdateMapFromProblemToTrainingStatus
...
def test_eim_approximation_19(expression_type, basis_generation):
    """
    This test is similar to test 15. The difference with test 15 is that the (solution dependent) function to be interpolated is now the gradient of a component of the solution, resulting in a (solution dependent) coefficent which is an UFL Grad operator.
    """
    ...
    class MockProblem(ParametrizedProblem):
        def __init__(self, V, **kwargs):
            # Call parent
            ParametrizedProblem.__init__(self, os.path.join("test_eim_approximation_19_tempdir", expression_type, basis_generation, "mock_problem"))
        ...
        def name(self):
            return "MockProblem_19_" + expression_type + "_" + basis_generation
    ...
    class MockReductionMethod(ReductionMethod):
        def __init__(self, truth_problem, **kwargs):
            # Call parent
            ReductionMethod.__init__(self, os.path.join("test_eim_approximation_19_tempdir", expression_type, basis_generation, "mock_problem"))
    ...
    class ParametrizedFunctionApproximation(EIMApproximation):
        def __init__(self, truth_problem, expression_type, basis_generation):
            ...
            #
            folder_prefix = os.path.join("test_eim_approximation_19_tempdir", expression_type, basis_generation)
            assert expression_type in ("Function", "Vector", "Matrix")
            if expression_type == "Function":
                # Call Parent constructor
                EIMApproximation.__init__(self, truth_problem, ParametrizedExpressionFactory(grad(f0)), folder_prefix, basis_generation)
