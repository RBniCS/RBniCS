...
class ParametrizedFunctionApproximation(EIMApproximation):
    def __init__(self, V, expression_type, basis_generation):
        ...
        if expression_type == "Function":
            ...
            EIMApproximation.__init__(self, None, ParametrizedExpressionFactory(None, f), "test_eim_approximation_06_function.output_dir", basis_generation)
        elif expression_type == "Vector":
            v = TestFunction(V)
            form = inner(f, grad(v))*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ParametrizedTensorFactory(None, form), "test_eim_approximation_06_vector.output_dir", basis_generation)
        elif expression_type == "Matrix":
            u = TrialFunction(V)
            v = TestFunction(V)
            form = inner(f, grad(u))*v*dx
            # Call Parent constructor
            EIMApproximation.__init__(self, None, ParametrizedTensorFactory(None, form), "test_eim_approximation_06_matrix.output_dir", basis_generation)
