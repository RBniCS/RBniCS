...
def test_separated_parametrized_forms_scalar_1():
    ...
    test_logger.log(DEBUG, "This is a basic advection-diffusion-reaction parametrized form, with all parametrized coefficients")
    a1_sep.separate()
    ...
    assert "f_39 * f_38 * (sum_{i_8} (grad(v_0))[i_8] * (grad(v_1))[i_8] )" == str(a1_sep._form_with_placeholders[0].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_2():
    ...
    test_logger.log(DEBUG, "We move the diffusion coefficient inside the inner product, splitting it into two coefficients: note that coefficient extraction was forced to extract two coefficients because they were separated in the UFL tree")
    a2_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "sum_{i_{11}} ({ A | A_{i_9} = (grad(v_1))[i_9] * f_42 })[i_{11}] * ({ A | A_{i_{10}} = (grad(v_0))[i_{10}] * f_43 })[i_{11}] " == str(a2_sep._form_with_placeholders[0].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_3():
    ...
    test_logger.log(DEBUG, "\tCoefficients:")
    test_logger.log(DEBUG, "\t\t(" + str(a3_sep.coefficients[0][0]) + ", " + str(a3_sep.coefficients[0][1]) + ", " + str(a3_sep.coefficients[0][2]) + ")")
    ...
    test_logger.log(DEBUG, "\t\t(" + str(a3_sep.coefficients[5][0]) + ", " + str(a3_sep.coefficients[5][1]) + ", " + str(a3_sep.coefficients[5][2]) + ")")
    ...
    test_logger.log(DEBUG, "\tPlaceholders:")
    test_logger.log(DEBUG, "\t\t(" + str(a3_sep._placeholders[0][0]) + ", " + str(a3_sep._placeholders[0][1]) + ", " + str(a3_sep._placeholders[0][2]) + ")")
    ...
    test_logger.log(DEBUG, "\t\t(" + str(a3_sep._placeholders[5][0]) + ", " + str(a3_sep._placeholders[5][1]) + ", " + str(a3_sep._placeholders[5][2]) + ")")
    ...
    assert "f_48 * f_47 * f_46 * (sum_{i_{12}} (grad(v_0))[i_{12}] * (grad(v_1))[i_{12}] )" == str(a3_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "f_50 * f_49 * (sum_{i_{12}} (grad(v_0))[i_{12}] * (grad(v_1))[i_{12}] )" == str(a3_sep._form_with_placeholders[1].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_4():
    ...
    assert "sum_{i_{17}} ({ A | A_{i_{15}} = sum_{i_{16}} f_58[i_{15}, i_{16}] * (grad(v_1))[i_{16}]  })[i_{17}] * (grad(v_0))[i_{17}] " == str(a4_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "sum_{i_{17}} ({ A | A_{i_{15}} = sum_{i_{16}} f_59[i_{15}, i_{16}] * (grad(v_1))[i_{16}]  })[i_{17}] * (grad(v_0))[i_{17}] " == str(a4_sep._form_with_placeholders[1].integrals()[0].integrand())
    assert "v_0 * (sum_{i_{18}} f_60[i_{18}] * (grad(v_1))[i_{18}] )" == str(a4_sep._form_with_placeholders[2].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_5():
    ...
    assert "f_63 * f_62 * (sum_{i_{19}} (grad(v_0))[i_{19}] * (grad(v_1))[i_{19}] )" == str(a5_sep._form_with_placeholders[0].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_6():
    ...
    test_logger.log(DEBUG, "We add a term depending on the mesh size. The extracted coefficient does not retain the mesh size factor")
    a6_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_8():
    ...
    assert "f_67 * f_13 * (sum_{i_{21}} (grad(v_0))[i_{21}] * (grad(v_1))[i_{21}] )" == str(a8_sep._form_with_placeholders[0].integrals()[0].integrand())
    test_logger.log(DEBUG, "\tLen unchanged forms:")
...
def test_separated_parametrized_forms_scalar_9():
    ...
    test_logger.log(DEBUG, "A part of the diffusion coefficient is parametrized (advection-reaction are not parametrized), where the terms are written in a different way when compared to form 8. Due to the UFL tree this would entail to extract two (\"sub\") coefficients, but this is not done for the sake of efficiency")
    a9_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "f_68 * (sum_{i_{22}} (grad(v_0))[i_{22}] * (grad(v_1))[i_{22}] )" == str(a9_sep._form_with_placeholders[0].integrals()[0].integrand())
    test_logger.log(DEBUG, "\tLen unchanged forms:")
...
def test_separated_parametrized_forms_scalar_10():
    ...
    test_logger.log(DEBUG, "Similarly to form 6, we add a term depending on the mesh size multiplied by a non-parametrized coefficient. Neither are retained")
    a10_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_11():
    ...
    test_logger.log(DEBUG, "Similarly to form 6, we add a term depending on the mesh size multiplied by the product of parametrized and a non-parametrized coefficients. Neither the non-parametrized coefficient nor the mesh size are retained")
    a11_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_12():
    ...
    test_logger.log(DEBUG, "We change form 11 with a slightly differenc coefficient. In this case the extraction retains the mesh size")
    a12_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_13():
    ...
    assert "sum_{i_{31}} ({ A | A_{i_{28}} = sum_{i_{29}} ({ A | A_{i_{26}, i_{27}} = ({ A | A_{i_{23}, i_{24}} = sum_{i_{25}} f_15[i_{23}, i_{25}] * f_71[i_{25}, i_{24}]  })[i_{26}, i_{27}] * f_14 })[i_{28}, i_{29}] * (grad(v_1))[i_{29}]  })[i_{31}] * (grad(v_0))[i_{31}] " == str(a13_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "v_0 * (sum_{i_{32}} ({ A | A_{i_{30}} = f_72[i_{30}] * f_14 })[i_{32}] * (grad(v_1))[i_{32}] )" == str(a13_sep._form_with_placeholders[1].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_14():
    ...
    test_logger.log(DEBUG, "This form is similar to form 1, but each term is multiplied by a scalar Function, which is the solution of a parametrized problem")
    a14_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "f_79 * f_78 * (sum_{i_{33}} (grad(v_0))[i_{33}] * (grad(v_1))[i_{33}] )" == str(a14_sep._form_with_placeholders[0].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_15():
    ...
    test_logger.log(DEBUG, "This form is similar to form 4, but each term is replace by a solution of a parametrized problem, either scalar, vector or tensor shaped")
    a15_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "sum_{i_{36}} ({ A | A_{i_{34}} = sum_{i_{35}} f_83[i_{34}, i_{35}] * (grad(v_1))[i_{35}]  })[i_{36}] * (grad(v_0))[i_{36}] " == str(a15_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "v_0 * (sum_{i_{37}} f_84[i_{37}] * (grad(v_1))[i_{37}] )" == str(a15_sep._form_with_placeholders[1].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_16():
    ...
    test_logger.log(DEBUG, "We change the coefficients of form 14 to be non-parametrized (as in form 7). Due to the presence of solutions of parametrized problems in contrast to form 7 all coefficients are now parametrized.")
    a16_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "sum_{i_{44}} ({ A | A_{i_{42}} = sum_{i_{43}} ({ A | A_{i_{40}, i_{41}} = ({ A | A_{i_{38}, i_{39}} = f_86[i_{38}, i_{39}] * f_13 })[i_{40}, i_{41}] * f_12 })[i_{42}, i_{43}] * (grad(v_1))[i_{43}]  })[i_{44}] * (grad(v_0))[i_{44}] " == str(a16_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "v_0 * (sum_{i_{45}} f_87[i_{45}] * (grad(v_1))[i_{45}] )" == str(a16_sep._form_with_placeholders[1].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_17():
    ...
    test_logger.log(DEBUG, "This form is similar to form 15, but each term is multiplied to a component of a solution of a parametrized problem")
    a17_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_18():
    ...
    test_logger.log(DEBUG, "This form is similar to form 14, but each term is multiplied by a scalar Function, which is not the solution of a parametrized problem")
    a18_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "f_91 * f_29 * (sum_{i_{46}} (grad(v_0))[i_{46}] * (grad(v_1))[i_{46}] )" == str(a18_sep._form_with_placeholders[0].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_19():
    ...
    test_logger.log(DEBUG, "This form is similar to form 16, but each term is multiplied by a scalar Function, which is not the solution of a parametrized problem. As in form 7, no parametrized coefficients are detected")
    a19_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_20():
    ...
    test_logger.log(DEBUG, "This form is similar to form 17, but each term is multiplied to a component of a Function which is not the solution of a parametrized problem. This results in no parametrized coefficients")
    a20_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_21():
    ...
    test_logger.log(DEBUG, "This form is similar to form 17, but each term is multiplied to the gradient/partial derivative of a component of a solution of a parametrized problem")
    a21_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
    ...
    assert "v_0 * (sum_{i_{55}} f_94[0, i_{55}] * (grad(v_1))[i_{55}] )" == str(a21_sep._form_with_placeholders[0].integrals()[0].integrand())
...
def test_separated_parametrized_forms_scalar_22():
    ...
    test_logger.log(DEBUG, "This form is similar to form 21, but each term is multiplied to the gradient/partial derivative of a component of a Function which is not the solution of a parametrized problem. This results in no parametrized coefficients")
    a22_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_23():
    ...
    test_logger.log(DEBUG, "This form tests a division between two expressions, which can be collected as one coefficient")
    a23_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
...
def test_separated_parametrized_forms_scalar_24():
    ...
    test_logger.log(DEBUG, "This form tests a division between two expressions, which (in contrast to form 24) cannot be collected as one coefficient")
    a24_sep.separate()
    test_logger.log(DEBUG, "\tLen coefficients:")
