...
from dolfin import CellDiameter, Constant, det, div, dx, Expression, FiniteElement, Function, FunctionSpace, grad, has_pybind11, inner, MixedElement, split, TensorFunctionSpace, TestFunction, tr, TrialFunction, UnitSquareMesh, VectorElement, VectorFunctionSpace
if has_pybind11():
...
def test_separated_parametrized_forms_mixed_1():
    ...
    assert "f_5" == str(a1_sep.coefficients[4][0])
    ...
    assert "-1 * v_0[2] * f_48 * (sum_{i_{21}} ([{ A | A_{i_{30}} = (grad(v_1))[0, i_{30}] }, { A | A_{i_{31}} = (grad(v_1))[1, i_{31}] }])[i_{21}, i_{21}] )" == str(a1_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_49" == str(a1_sep._form_with_placeholders[5].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_2():
    ...
    assert "f_5" == str(a2_sep.coefficients[4][0])
    ...
    assert "-1 * v_0[2] * f_54 * (sum_{i_{49}} ([{ A | A_{i_{58}} = (grad(v_1))[0, i_{58}] }, { A | A_{i_{59}} = (grad(v_1))[1, i_{59}] }])[i_{49}, i_{49}] )" == str(a2_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_55" == str(a2_sep._form_with_placeholders[5].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_3():
    ...
    assert "{ A | A_{i_{66}, i_{67}} = ({ A | A_{i_{64}, i_{65}} = (({ A | A_{i_{61}, i_{62}} = sum_{i_{63}} f_7[i_{61}, i_{63}] * f_7[i_{63}, i_{62}]  }) + f_8)[i_{64}, i_{65}] * (f_7[0, 0] * f_7[1, 1] + -1 * f_7[0, 1] * f_7[1, 0]) })[i_{66}, i_{67}] * f_5 }" == str(a3_sep.coefficients[0][0])
    assert "f_6" == str(a3_sep.coefficients[1][0])
    assert "f_5" == str(a3_sep.coefficients[2][0])
    assert "f_8" == str(a3_sep.coefficients[3][0])
    assert "f_5" == str(a3_sep.coefficients[4][0])
    assert "f_6[0]" == str(a3_sep.coefficients[5][0])
    log(PROGRESS, "\tPlaceholders:\n" +
    ...
    assert "sum_{i_{78}} ([v_0[0], v_0[1]])[i_{78}] * ({ A | A_{i_{71}} = sum_{i_{72}} ([{ A | A_{i_{86}} = (grad(v_1))[0, i_{86}] }, { A | A_{i_{87}} = (grad(v_1))[1, i_{87}] }])[i_{71}, i_{72}] * f_57[i_{72}]  })[i_{78}] " == str(a3_sep._form_with_placeholders[1].integrals()[0].integrand())
    assert "f_58 * (sum_{i_{79}} ([v_0[0], v_0[1]])[i_{79}] * ([v_1[0], v_1[1]])[i_{79}] )" == str(a3_sep._form_with_placeholders[2].integrals()[0].integrand())
    assert "-1 * v_1[2] * (sum_{i_{80}} ({ A | A_{i_{73}, i_{74}} = sum_{i_{75}} ([{ A | A_{i_{88}} = (grad(v_0))[0, i_{88}] }, { A | A_{i_{89}} = (grad(v_0))[1, i_{89}] }])[i_{75}, i_{74}] * f_59[i_{73}, i_{75}]  })[i_{80}, i_{80}] )" == str(a3_sep._form_with_placeholders[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_60 * (sum_{i_{81}} ([{ A | A_{i_{90}} = (grad(v_1))[0, i_{90}] }, { A | A_{i_{91}} = (grad(v_1))[1, i_{91}] }])[i_{81}, i_{81}] )" == str(a3_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_61" == str(a3_sep._form_with_placeholders[5].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_4():
    ...
    assert "diameter * f_5" == str(a4_sep.coefficients[4][0])
    assert "diameter * f_6[0]" == str(a4_sep.coefficients[5][0])
    log(PROGRESS, "\tPlaceholders:\n" +
    ...
    assert "f_62" == str(a4_sep._placeholders[0][0])
    assert "f_63" == str(a4_sep._placeholders[1][0])
    assert "f_64" == str(a4_sep._placeholders[2][0])
    assert "f_65" == str(a4_sep._placeholders[3][0])
    assert "f_66" == str(a4_sep._placeholders[4][0])
    assert "f_67" == str(a4_sep._placeholders[5][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
    ...
    assert "sum_{i_{107}} sum_{i_{106}} ([{ A | A_{i_{114}} = (grad(v_0))[0, i_{114}] }, { A | A_{i_{115}} = (grad(v_0))[1, i_{115}] }])[i_{106}, i_{107}] * ({ A | A_{i_{95}, i_{96}} = sum_{i_{97}} ([{ A | A_{i_{112}} = (grad(v_1))[0, i_{112}] }, { A | A_{i_{113}} = (grad(v_1))[1, i_{113}] }])[i_{97}, i_{96}] * f_62[i_{95}, i_{97}]  })[i_{106}, i_{107}]  " == str(a4_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "sum_{i_{108}} ([v_0[0], v_0[1]])[i_{108}] * ({ A | A_{i_{100}} = diameter * ({ A | A_{i_{98}} = sum_{i_{99}} ([{ A | A_{i_{116}} = (grad(v_1))[0, i_{116}] }, { A | A_{i_{117}} = (grad(v_1))[1, i_{117}] }])[i_{98}, i_{99}] * f_63[i_{99}]  })[i_{100}] })[i_{108}] " == str(a4_sep._form_with_placeholders[1].integrals()[0].integrand())
    assert "f_64 * (sum_{i_{109}} ([v_0[0], v_0[1]])[i_{109}] * ([v_1[0], v_1[1]])[i_{109}] )" == str(a4_sep._form_with_placeholders[2].integrals()[0].integrand())
    assert "-1 * v_1[2] * (sum_{i_{110}} ({ A | A_{i_{103}, i_{104}} = sum_{i_{105}} ([{ A | A_{i_{118}} = (grad(v_0))[0, i_{118}] }, { A | A_{i_{119}} = (grad(v_0))[1, i_{119}] }])[i_{105}, i_{104}] * f_65[i_{103}, i_{105}]  })[i_{110}, i_{110}] )" == str(a4_sep._form_with_placeholders[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_66 * (sum_{i_{111}} ([{ A | A_{i_{120}} = (grad(v_1))[0, i_{120}] }, { A | A_{i_{121}} = (grad(v_1))[1, i_{121}] }])[i_{111}, i_{111}] )" == str(a4_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_67" == str(a4_sep._form_with_placeholders[5].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_5():
    ...
    assert "diameter * f_5" == str(a5_sep.coefficients[4][0])
    assert "diameter * f_6[0]" == str(a5_sep.coefficients[5][0])
    log(PROGRESS, "\tPlaceholders:\n" +
    ...
    assert "f_68" == str(a5_sep._placeholders[0][0])
    assert "f_69" == str(a5_sep._placeholders[1][0])
    assert "f_70" == str(a5_sep._placeholders[2][0])
    assert "f_71" == str(a5_sep._placeholders[3][0])
    assert "f_72" == str(a5_sep._placeholders[4][0])
    assert "f_73" == str(a5_sep._placeholders[5][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
    ...
    assert "sum_{i_{137}} sum_{i_{136}} ([{ A | A_{i_{144}} = (grad(v_0))[0, i_{144}] }, { A | A_{i_{145}} = (grad(v_0))[1, i_{145}] }])[i_{136}, i_{137}] * ({ A | A_{i_{125}, i_{126}} = sum_{i_{127}} ([{ A | A_{i_{142}} = (grad(v_1))[0, i_{142}] }, { A | A_{i_{143}} = (grad(v_1))[1, i_{143}] }])[i_{127}, i_{126}] * f_68[i_{125}, i_{127}]  })[i_{136}, i_{137}]  " == str(a5_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "sum_{i_{138}} ([v_0[0], v_0[1]])[i_{138}] * ({ A | A_{i_{129}} = sum_{i_{130}} ([{ A | A_{i_{146}} = (grad(v_1))[0, i_{146}] }, { A | A_{i_{147}} = (grad(v_1))[1, i_{147}] }])[i_{129}, i_{130}] * f_69[i_{130}]  })[i_{138}] " == str(a5_sep._form_with_placeholders[1].integrals()[0].integrand())
    assert "f_70 * (sum_{i_{139}} ([v_0[0], v_0[1]])[i_{139}] * ([v_1[0], v_1[1]])[i_{139}] )" == str(a5_sep._form_with_placeholders[2].integrals()[0].integrand())
    assert "-1 * v_1[2] * (sum_{i_{140}} ({ A | A_{i_{133}, i_{134}} = sum_{i_{135}} ([{ A | A_{i_{148}} = (grad(v_0))[0, i_{148}] }, { A | A_{i_{149}} = (grad(v_0))[1, i_{149}] }])[i_{135}, i_{134}] * f_71[i_{133}, i_{135}]  })[i_{140}, i_{140}] )" == str(a5_sep._form_with_placeholders[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_72 * (sum_{i_{141}} ([{ A | A_{i_{150}} = (grad(v_1))[0, i_{150}] }, { A | A_{i_{151}} = (grad(v_1))[1, i_{151}] }])[i_{141}, i_{141}] )" == str(a5_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_73" == str(a5_sep._form_with_placeholders[5].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_6():
    ...
    assert "-1 * v_0[2] * f_9 * (sum_{i_{166}} ([{ A | A_{i_{175}} = (grad(v_1))[0, i_{175}] }, { A | A_{i_{176}} = (grad(v_1))[1, i_{176}] }])[i_{166}, i_{166}] )" == str(a6_sep._form_unchanged[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_10[0] * v_1[2]" == str(a6_sep._form_unchanged[5].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_7():
    ...
    assert "f_74" == str(a7_sep._placeholders[0][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a7_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n"
        )
    assert "sum_{i_{193}} sum_{i_{192}} ([{ A | A_{i_{200}} = (grad(v_0))[0, i_{200}] }, { A | A_{i_{201}} = (grad(v_0))[1, i_{201}] }])[i_{192}, i_{193}] * ({ A | A_{i_{184}, i_{185}} = sum_{i_{186}} ([{ A | A_{i_{198}} = (grad(v_1))[0, i_{198}] }, { A | A_{i_{199}} = (grad(v_1))[1, i_{199}] }])[i_{186}, i_{185}] * ({ A | A_{i_{181}, i_{182}} = sum_{i_{183}} f_11[i_{181}, i_{183}] * f_74[i_{183}, i_{182}]  })[i_{184}, i_{186}]  })[i_{192}, i_{193}]  " == str(a7_sep._form_with_placeholders[0].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
    ...
    assert "-1 * v_0[2] * f_9 * (sum_{i_{197}} ([{ A | A_{i_{206}} = (grad(v_1))[0, i_{206}] }, { A | A_{i_{207}} = (grad(v_1))[1, i_{207}] }])[i_{197}, i_{197}] )" == str(a7_sep._form_unchanged[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_10[0] * v_1[2]" == str(a7_sep._form_unchanged[4].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_8():
    ...
    log(PROGRESS, "*** ###              FORM 8             ### ***")
    log(PROGRESS, "This case is similar to form 7, but the order of the matrix multiplication is different. In order not to extract separately f_7 and f_8, the whole product (even with the non-parametrized part) is extracted.")
    a8_sep.separate()
    log(PROGRESS, "\tLen coefficients:\n" +
    ...
    assert "-1 * v_0[2] * f_9 * (sum_{i_{228}} ([{ A | A_{i_{237}} = (grad(v_1))[0, i_{237}] }, { A | A_{i_{238}} = (grad(v_1))[1, i_{238}] }])[i_{228}, i_{228}] )" == str(a8_sep._form_unchanged[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_10[0] * v_1[2]" == str(a8_sep._form_unchanged[4].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_9():
    ...
    assert "f_76" == str(a9_sep._placeholders[0][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a9_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n"
        )
    assert "sum_{i_{255}} sum_{i_{254}} ([{ A | A_{i_{262}} = (grad(v_0))[0, i_{262}] }, { A | A_{i_{263}} = (grad(v_0))[1, i_{263}] }])[i_{254}, i_{255}] * ({ A | A_{i_{246}, i_{247}} = sum_{i_{248}} ([{ A | A_{i_{260}} = (grad(v_1))[0, i_{260}] }, { A | A_{i_{261}} = (grad(v_1))[1, i_{261}] }])[i_{248}, i_{247}] * ({ A | A_{i_{243}, i_{244}} = sum_{i_{245}} f_13[i_{243}, i_{245}] * f_76[i_{245}, i_{244}]  })[i_{246}, i_{248}]  })[i_{254}, i_{255}]  " == str(a9_sep._form_with_placeholders[0].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
    ...
    assert "-1 * v_0[2] * f_9 * (sum_{i_{259}} ([{ A | A_{i_{268}} = (grad(v_1))[0, i_{268}] }, { A | A_{i_{269}} = (grad(v_1))[1, i_{269}] }])[i_{259}, i_{259}] )" == str(a9_sep._form_unchanged[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_10[0] * v_1[2]" == str(a9_sep._form_unchanged[4].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_10():
    ...
    assert "f_77" == str(a10_sep._placeholders[0][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a10_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n"
        )
    assert "sum_{i_{286}} sum_{i_{285}} ([{ A | A_{i_{293}} = (grad(v_0))[0, i_{293}] }, { A | A_{i_{294}} = (grad(v_0))[1, i_{294}] }])[i_{285}, i_{286}] * ({ A | A_{i_{277}, i_{278}} = sum_{i_{279}} ([{ A | A_{i_{291}} = (grad(v_1))[0, i_{291}] }, { A | A_{i_{292}} = (grad(v_1))[1, i_{292}] }])[i_{279}, i_{278}] * f_77[i_{277}, i_{279}]  })[i_{285}, i_{286}]  " == str(a10_sep._form_with_placeholders[0].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
    ...
    assert "-1 * v_0[2] * f_9 * (sum_{i_{290}} ([{ A | A_{i_{299}} = (grad(v_1))[0, i_{299}] }, { A | A_{i_{300}} = (grad(v_1))[1, i_{300}] }])[i_{290}, i_{290}] )" == str(a10_sep._form_unchanged[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_10[0] * v_1[2]" == str(a10_sep._form_unchanged[4].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_11():
    ...
    assert "f_20" == str(a11_sep.coefficients[4][0])
    ...
    assert "f_90" == str(a11_sep._placeholders[0][0])
    assert "f_91" == str(a11_sep._placeholders[1][0])
    assert "f_92" == str(a11_sep._placeholders[2][0])
    assert "f_93" == str(a11_sep._placeholders[3][0])
    assert "f_94" == str(a11_sep._placeholders[4][0])
    assert "f_95" == str(a11_sep._placeholders[5][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
    ...
    assert "sum_{i_{311}} sum_{i_{310}} ([{ A | A_{i_{318}} = (grad(v_0))[0, i_{318}] }, { A | A_{i_{319}} = (grad(v_0))[1, i_{319}] }])[i_{310}, i_{311}] * ({ A | A_{i_{302}, i_{303}} = sum_{i_{304}} ([{ A | A_{i_{316}} = (grad(v_1))[0, i_{316}] }, { A | A_{i_{317}} = (grad(v_1))[1, i_{317}] }])[i_{304}, i_{303}] * f_90[i_{302}, i_{304}]  })[i_{310}, i_{311}]  " == str(a11_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "sum_{i_{312}} ([v_0[0], v_0[1]])[i_{312}] * ({ A | A_{i_{305}} = sum_{i_{306}} ([{ A | A_{i_{320}} = (grad(v_1))[0, i_{320}] }, { A | A_{i_{321}} = (grad(v_1))[1, i_{321}] }])[i_{305}, i_{306}] * f_91[i_{306}]  })[i_{312}] " == str(a11_sep._form_with_placeholders[1].integrals()[0].integrand())
    assert "f_92 * (sum_{i_{313}} ([v_0[0], v_0[1]])[i_{313}] * ([v_1[0], v_1[1]])[i_{313}] )" == str(a11_sep._form_with_placeholders[2].integrals()[0].integrand())
    assert "-1 * v_1[2] * (sum_{i_{314}} ({ A | A_{i_{307}, i_{308}} = sum_{i_{309}} ([{ A | A_{i_{322}} = (grad(v_0))[0, i_{322}] }, { A | A_{i_{323}} = (grad(v_0))[1, i_{323}] }])[i_{309}, i_{308}] * f_93[i_{307}, i_{309}]  })[i_{314}, i_{314}] )" == str(a11_sep._form_with_placeholders[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_94 * (sum_{i_{315}} ([{ A | A_{i_{324}} = (grad(v_1))[0, i_{324}] }, { A | A_{i_{325}} = (grad(v_1))[1, i_{325}] }])[i_{315}, i_{315}] )" == str(a11_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_95" == str(a11_sep._form_with_placeholders[5].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_12():
    ...
    assert "f_96" == str(a12_sep._placeholders[0][0])
    assert "f_97" == str(a12_sep._placeholders[1][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a12_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n" +
        "\t\t" + str(a12_sep._form_with_placeholders[1].integrals()[0].integrand()) + "\n"
        )
    assert "v_0 * v_1 * f_96" == str(a12_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "v_0 * (grad(v_1))[0] * f_97" == str(a12_sep._form_with_placeholders[1].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_13():
    ...
    assert "f_98" == str(a13_sep._placeholders[0][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a13_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n"
        )
    assert "v_0 * (sum_{i_{327}} f_98[i_{327}] * (grad(v_1))[i_{327}] )" == str(a13_sep._form_with_placeholders[0].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_14():
    ...
    assert "-1 * v_0[2] * f_32 * (sum_{i_{341}} ([{ A | A_{i_{350}} = (grad(v_1))[0, i_{350}] }, { A | A_{i_{351}} = (grad(v_1))[1, i_{351}] }])[i_{341}, i_{341}] )" == str(a14_sep._form_unchanged[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * f_35[0] * v_1[2]" == str(a14_sep._form_unchanged[5].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_15():
    ...
    assert "v_0 * (grad(v_1))[0] * f_41[1]" == str(a15_sep._form_unchanged[1].integrals()[0].integrand())
...
def test_separated_parametrized_forms_mixed_17():
    ...
    assert "f_99" == str(a17_sep._placeholders[0][0])
    assert "f_100" == str(a17_sep._placeholders[1][0])
    assert "f_101" == str(a17_sep._placeholders[2][0])
    assert "f_102" == str(a17_sep._placeholders[3][0])
    assert "f_103" == str(a17_sep._placeholders[4][0])
    assert "f_104" == str(a17_sep._placeholders[5][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
    ...
    assert "sum_{i_{363}} sum_{i_{362}} ([{ A | A_{i_{371}} = (grad(v_0))[0, i_{371}] }, { A | A_{i_{372}} = (grad(v_0))[1, i_{372}] }])[i_{362}, i_{363}] * ({ A | A_{i_{354}, i_{355}} = sum_{i_{356}} ([{ A | A_{i_{369}} = (grad(v_1))[0, i_{369}] }, { A | A_{i_{370}} = (grad(v_1))[1, i_{370}] }])[i_{356}, i_{355}] * f_99[i_{354}, i_{356}]  })[i_{362}, i_{363}]  " == str(a17_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "sum_{i_{364}} ([v_0[0], v_0[1]])[i_{364}] * ({ A | A_{i_{357}} = sum_{i_{358}} ([{ A | A_{i_{373}} = (grad(v_1))[0, i_{373}] }, { A | A_{i_{374}} = (grad(v_1))[1, i_{374}] }])[i_{357}, i_{358}] * f_100[i_{358}]  })[i_{364}] " == str(a17_sep._form_with_placeholders[1].integrals()[0].integrand())
    assert "f_101 * (sum_{i_{365}} ([v_0[0], v_0[1]])[i_{365}] * ([v_1[0], v_1[1]])[i_{365}] )" == str(a17_sep._form_with_placeholders[2].integrals()[0].integrand())
    assert "-1 * v_1[2] * (sum_{i_{366}} ({ A | A_{i_{359}, i_{360}} = sum_{i_{361}} ([{ A | A_{i_{375}} = (grad(v_0))[0, i_{375}] }, { A | A_{i_{376}} = (grad(v_0))[1, i_{376}] }])[i_{361}, i_{360}] * f_102[i_{359}, i_{361}]  })[i_{366}, i_{366}] )" == str(a17_sep._form_with_placeholders[3].integrals()[0].integrand())
    assert "-1 * v_0[2] * (sum_{i_{367}} f_103[i_{367}, i_{367}] ) * (sum_{i_{368}} ([{ A | A_{i_{378}} = (grad(v_1))[0, i_{378}] }, { A | A_{i_{379}} = (grad(v_1))[1, i_{379}] }])[i_{368}, i_{368}] )" == str(a17_sep._form_with_placeholders[4].integrals()[0].integrand())
    assert "-1 * v_0[2] * v_1[2] * f_104" == str(a17_sep._form_with_placeholders[5].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_18():
    ...
    assert "f_105" == str(a18_sep._placeholders[0][0])
    assert "f_106" == str(a18_sep._placeholders[1][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a18_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n" +
        "\t\t" + str(a18_sep._form_with_placeholders[1].integrals()[0].integrand()) + "\n"
        )
    assert "v_0 * (sum_{i_{381}} f_105[0, i_{381}] * (grad(v_1))[i_{381}] )" == str(a18_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "v_0 * (grad(v_1))[0] * f_106" == str(a18_sep._form_with_placeholders[1].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_19():
    ...
    assert "f_107" == str(a19_sep._placeholders[0][0])
    assert "f_108" == str(a19_sep._placeholders[1][0])
    log(PROGRESS, "\tForms with placeholders:\n" +
        "\t\t" + str(a19_sep._form_with_placeholders[0].integrals()[0].integrand()) + "\n" +
        "\t\t" + str(a19_sep._form_with_placeholders[1].integrals()[0].integrand()) + "\n"
        )
    assert "sum_{i_{387}} ({ A | A_{i_{384}} = sum_{i_{385}} ([{ A | A_{i_{389}} = f_107[0, i_{389}] }, { A | A_{i_{390}} = f_107[1, i_{390}] }])[i_{384}, i_{385}] * (grad(v_1))[i_{385}]  })[i_{387}] * (grad(v_0))[i_{387}] " == str(a19_sep._form_with_placeholders[0].integrals()[0].integrand())
    assert "v_0 * (sum_{i_{388}} ([{ A | A_{i_{391}} = f_108[0, i_{391}] }, { A | A_{i_{392}} = f_108[1, i_{392}] }])[i_{388}, 0] * (grad(v_1))[i_{388}] )" == str(a19_sep._form_with_placeholders[1].integrals()[0].integrand())
    log(PROGRESS, "\tLen unchanged forms:\n" +
...
def test_separated_parametrized_forms_mixed_20():
    ...
    assert "-1 * v_0[2] * (grad(f_32))[1] * v_1[2]" == str(a20_sep._form_unchanged[5].integrals()[0].integrand())
